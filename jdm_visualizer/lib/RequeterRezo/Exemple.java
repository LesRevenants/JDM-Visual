public class Exemple {
	...
	public static void main(String[] args) {
		//Délais à partir duquel un mot du cache doit être redemander au serveur
		...
		//Une requete peut renvoyer null si le serveur ne répond pas ou que le mot n'existe pas
		if(chaton != null) {
			System.out.println(chaton);		
			Mot.Ecrire(chaton, "chaton.txt");
		}

		//requête avec filtre sur le type de relation ainsi que sur la direction
		...
		//Uniquement les relations sortantes car les relations entrantes ont été filtrées 
		for(Voisin voisin : chatAvecFiltre.getRelations_sortantes("r_isa")) {
			System.out.println("\t"+voisin.toString());
		}

		//Possibilité de filtrer plusieurs types de relations
		...
		for(Mot chat : chatAvecMultiplesFiltres) {
			//traitement			
			System.out.println("\t"+chat.getVoisinage().size() + " éléments");
		}

		//à ne pas oublier : essentiel pour la gestion du cache !
		rezo.sauvegarder();
	}
